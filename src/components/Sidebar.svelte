<script>
  ...
  function loop() {
    ...
    const keys = Object.keys( window.callbacks )
    const k = keys[a]
    const send = window.callbacks[k];
    const e = emotions[b]
...
<style lang="sass">
  ...
  <div class="actions flex flex-column">
    ...
    <div class="select flex mb0-4">
      <select bind:value={viz.title} class="b1-solid flex grow">
    ...
    <Checkbox className="mb0-6" bind:checked={viz.wait}>wait</Checkbox>
    <input class="mb0-4" type="text" placeholder="message" bind:value={viz.message} />
    <input class="mb0-4" type="text" placeholder="button" bind:value={viz.button} />
    <input class="mb0-4" type="number" placeholder="timer" bind:value={viz.timer} />
    <button class="mb1" on:click={sendAll} >send to all</button>
    <div class="bright pb1 mb1 mt2 bb1-solid">Test</div>
    <input type="range" bind:value={speed} min="10" max="1000" /> {speed}
